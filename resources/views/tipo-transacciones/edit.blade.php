...
@section('content')
    ...
    <form action="{{ route('tipo-transacciones.update',$tt) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method("PUT")
        <div class="card">
                ...
                <div class="col-md-6 mb-1">
                    <div class="form-floating form-control-feedback form-control-feedback-start">
                        ...
                        <select class="form-select @error('tipo') is-invalid @enderror" name="tipo" required>
                            <option value=""></option>
                            <option value="INGRESO" {{ old('tipo',$tt->tipo)=='INGRESO'?'selected':'' }}>INGRESO</option>
                            <option value="SALIDA" {{ old('tipo',$tt->tipo)=='SALIDA'?'selected':'' }}>SALIDA</option>
                            <option value="DEBITO" {{ old('tipo',$tt->tipo)=='DEBITO'?'selected':'' }}>DEBITO</option>
                            <option value="ABONO" {{ old('tipo',$tt->tipo)=='ABONO'?'selected':'' }}>ABONO</option>
                        </select>
