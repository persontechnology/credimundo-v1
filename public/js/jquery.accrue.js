...
;(function( $, window, document, undefined ){
    ...
    $.fn.accrue.options = {
        ...
        field_comments: {
            ...
            rate: "",
            rate_compare: "",
            term: "Formato: 12m, 36m, 3y, 7y"
        },
        response_output_div: ".results",
        response_basic: 
            '<p><strong>Mensualidad:</strong><br />%payment_amount%</p>'+
            '<p><strong>Numero de pagos:</strong><br />%num_payments%</p>'+
    ...
    var get_field = function( elem, options, name ) {
        ...
        elem.find(".form").append(
            '<div class="accrue-field-'+name+' col-sm-4">'+
                '<p><label>'+options.field_titles[name]+':</label>'+
                '<input type="text" name="'+name+'" class="'+name+' form-control" value="'+options.default_values[name]+'" />'+
    ...
    var calculateAmortization = function( elem, options, output_elem ){
        ...
        if ( loan_info!==0 ) {
            ...
            var output_content = '<table class="accrue-amortization table table-sm table-bordered">'+
                    '<thead><tr>'+
                    '<th class="accrue-payment-number">#</th>'+
                    '<th class="accrue-fecha-pago">Fecha de pago</th>'+
                    '<th class="accrue-monto-pago">Monto de pago</th>'+
                    '<th class="accrue-ahorro-programado">Ahorro Programado</th>'+
                    ...
                    '<th class="accrue-balance">Total a pagar</th>'+
                    '</tr></thead><tbody>',
                ...
                // para dia de pago
                var fecha_pago=moment($('#dia_pago').val());
                var diaPago=fecha_pago.date();
                var siguenteMes=fecha_pago;
                // ahorro programado
                var ahorro_programado=$('#ahorro_programado').val();
            ...
            for ( var i=0; i<loan_info.num_payments; i++) { 
                ...
                // config
                var monto_f=  Math.abs(loan_info.payment_amount_formatted).toFixed(2);
                var interest_f=  Math.abs(counter_interest).toFixed(2);
                var pagos_f=  Math.abs(counter_payment).toFixed(2);
                var balance_f=  Math.abs(counter_balance).toFixed(2);
                var total_pagar=Math.abs(parseFloat(ahorro_programado)+parseFloat(monto_f)).toFixed(2);
                ...
                '<tr>'+
                '<'+cell_tag+' class="accrue-payment-number"><input type="hidden" name="numero_pago_tabla['+(i+1)+']" value="'+(i+1)+'" />'+(i+1)+'</'+cell_tag+'>'+
                '<'+cell_tag+' class="accrue-fecha-pago"><input type="hidden" value="'+fecha_pago.format('YYYY-MM-DD')+'" name="fecha_pago_tabla['+(i+1)+']" />'+fecha_pago.format('LL')+'</'+cell_tag+'>'+
                '<'+cell_tag+' class="accrue-payment-amount"><input type="hidden" value="'+monto_f+'" name="monto_pago_tabla['+(i+1)+']" />'+monto_f+'</'+cell_tag+'>'+
                '<'+cell_tag+' class="accrue-ahorro-programado">'+ahorro_programado+'</'+cell_tag+'>'+
                '<'+cell_tag+' class="accrue-total-interest"><input type="hidden" value="'+interest_f+'" name="interes_total_tabla['+(i+1)+']"/>'+interest_f+'</'+cell_tag+'>'+
                '<'+cell_tag+' class="accrue-total-payments"><input type="hidden" value="'+pagos_f+'" name="pagos_total_tabla['+(i+1)+']"/>'+pagos_f+'</'+cell_tag+'>'+
                '<'+cell_tag+' class="accrue-balance"><input type="hidden" value="'+balance_f+'" name="saldo_capital_tabla['+(i+1)+']"/>'+balance_f+'</'+cell_tag+'>'+
                '<'+cell_tag+' class="accrue-balance"><input type="hidden" value="'+total_pagar+'" name="total_a_pagar_tabla['+(i+1)+']"/>'+total_pagar+'</'+cell_tag+'>'+
                '</tr>';
            }
